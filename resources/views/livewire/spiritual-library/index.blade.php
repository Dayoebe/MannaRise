            ...
            <div class="grid gap-4 md:grid-cols-3">
                @foreach ($featuredBooks as $book)
                    <a href="{{ route('library.show', $book->slug) }}" class="app-panel border-t-4 border-cyan-200 border-t-cyan-500 hover:border-cyan-400 even:border-t-orange-500">
                        <p class="inline-flex items-center gap-2 rounded-full bg-cyan-50 px-3 py-1 text-xs font-bold uppercase tracking-normal text-cyan-900">📘 {{ $book->tradition }}</p>
                        ...
                        <p class="mt-4 inline-flex items-center gap-2 rounded-full bg-amber-100 px-3 py-1 text-sm font-bold text-amber-900">📖 {{ $book->chapters_count }} chapters</p>
                    </a>
                @endforeach
            </div>
        </section>
            ...
            @forelse ($books as $book)
                <a href="{{ route('library.show', $book->slug) }}" class="app-panel border-slate-200 hover:border-emerald-300 hover:bg-emerald-50">
                    <div class="flex items-start justify-between gap-3">
                        <p class="inline-flex items-center gap-2 rounded-full bg-teal-50 px-3 py-1 text-xs font-bold uppercase tracking-normal text-teal-900">📚 {{ $book->tradition }}</p>
